...
const sendResetPassword = async (user, token) => {
  ...
  const link = `${process.env.FRONTEND_URL}/api/v1/auth/reset-password?token=${token}&id=${user.id}`;
  ...
  try {
    await transporter.sendMail(mailOptions);
    logger.info(`Reset password link sent to ${user.email}`);
  } catch (error) {
    logger.error(`Failed to send reset password link to ${user.email}`, {
      error: error.message,
      code: error.code,
      command: error.command,
      response: error.response,
    ...
    throw new Error(`Failed to reset password link: ${error.message}`);
  }
};

module.exports = { sendVerificationEmail, sendResetPassword, verifyTransporter };
